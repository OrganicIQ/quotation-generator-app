...
const cors = require('cors'); // <-- Requiring the package
...
app.use(express.json()); // 2. Then use middleware

// 3. Then use the CORS middleware
app.use(cors({
    origin: 'https://quote.yourdomain.com', // IMPORTANT: Replace with your actual Hostinger URL
    credentials: true
}));
// ==========================================================
...
app.get('/api/quotes/:id/download', ensureAuth, async (req, res) => { try { const quote = await Quote.findById(req.params.id).populate('lineItems.product').populate('user'); if (!quote || quote.user._id.toString() !== req.user.id) { return res.status(404).send('Quote not found'); } const quoteDate = new Date(quote.createdAt).toLocaleDateString(); const itemsHtml = quote.lineItems.map((item, index) => { const product = item.product; const basePrice = item.priceAtTime; const quantity = item.quantity; const discount = item.discountPercentage; const discountedPrice = basePrice * (1 - (discount / 100)); const total = discountedPrice * quantity; const itemNameHtml = `<strong>${product.baseName} ${product.variantName || ''}</strong><br><small style="color:#555;">${product.description || ''}</small>`; return `<tr><td>${index + 1}</td><td>${itemNameHtml}</td><td>₹${basePrice.toFixed(2)}</td><td>${quantity}</td><td>${discount}%</td><td>₹${discountedPrice.toFixed(2)}</td><td>₹${total.toFixed(2)}</td></tr>`; }).join(''); const totalsHtml = `<tr><td colspan="6" style="text-align:right;">Subtotal:</td><td style="text-align:right;">₹${quote.subtotal.toFixed(2)}</td></tr>${quote.couponDiscountPercentage > 0 ? `<tr><td colspan="6" style="text-align:right;">Discount (${quote.couponDiscountPercentage}%):</td><td style="text-align:right;">- ₹${quote.couponDiscountAmount.toFixed(2)}</td></tr>` : ''}<tr><td colspan="6" style="text-align:right;">GST (${quote.gstPercentage}%):</td><td style="text-align:right;">+ ₹${quote.gstAmount.toFixed(2)}</td></tr><tr style="font-weight:bold; border-top: 2px solid #333;"><td colspan="6" style="text-align:right;">Grand Total:</td><td style="text-align:right;">₹${quote.grandTotal.toFixed(2)}</td></tr>`; const htmlContent = `<html><head><style>body{font-family:Helvetica,Arial,sans-serif;font-size:12px;}.invoice-box{max-width:800px;margin:auto;padding:30px;border:1px solid #eee;box-shadow:0 0 10px rgba(0,0,0,.15);}.header{text-align:center;margin-bottom:20px;}.items-table{width:100%;border-collapse:collapse;}.items-table th,.items-table td{border-bottom:1px solid #eee;padding:8px;}.items-table th{background-color:#f9f9f9;}.totals-table{width:100%;margin-top:20px;}.align-right{text-align:right;}.grand-total-row td{border-top:2px solid #333;font-weight:700}</style></head><body><div class="invoice-box"><div class="header"><h1>Quotation</h1><p>From: ${quote.user.displayName}</p></div><div><table><tr><td><strong>Quote No:</strong> ${quote.quoteNumber}</td></tr><tr><td><strong>Date:</strong> ${quoteDate}</td></tr><tr><td><strong>Client:</strong> ${quote.clientName}</td></tr></table></div><br><table class="items-table"><thead><tr><th>#</th><th>Item</th><th>Base Price</th><th>Qty</th><th>Disc %</th><th>Disc Price</th><th>Total</th></tr></thead><tbody>${itemsHtml}</tbody></table><table class="totals-table"><tbody>${totalsHtml}</tbody></table></div></body></html>`; const options = { format: 'A4', margin: { top: '0.5in', right: '0.5in', bottom: '0.5in', left: '0.5in' } }; const pdfBuffer = await html_to_pdf.generatePdf({ content: htmlContent }, options); res.setHeader('Content-Type', 'application/pdf'); res.setHeader('Content-Disposition', `attachment; filename=Quote-${quote.quoteNumber}.pdf`); res.send(pdfBuffer); } catch (err) { console.error('Error generating PDF:', err); res.status(500).send('Error'); } });
app.post('/api/quotes/preview-pdf', ensureAuth, async (req, res) => { try { const quoteData = req.body; const user = req.user; const quoteDate = new Date().toLocaleDateString(); const discount = 10; const itemsHtml = quoteData.lineItems.map((item, index) => { const basePrice = item.price; const quantity = item.quantity; const discountedPrice = basePrice * (1 - (discount / 100)); const total = discountedPrice * quantity; const itemNameHtml = item.name; return `<tr><td>${index + 1}</td><td>${itemNameHtml}</td><td>₹${basePrice.toFixed(2)}</td><td>${quantity}</td><td>${discount}%</td><td>₹${discountedPrice.toFixed(2)}</td><td>₹${total.toFixed(2)}</td></tr>`; }).join(''); const totalsHtml = `<tr><td colspan="6" style="text-align:right;">Grand Total:</td><td style="text-align:right;">₹${quoteData.grandTotal.toFixed(2)}</td></tr>`; const htmlContent = `<html><head><style>body{font-family:Helvetica,Arial,sans-serif;font-size:12px;}.invoice-box{max-width:800px;margin:auto;padding:30px;border:1px solid #eee;box-shadow:0 0 10px rgba(0,0,0,.15);}.header{text-align:center;margin-bottom:20px;}.items-table{width:100%;border-collapse:collapse;}.items-table th,.items-table td{border-bottom:1px solid #eee;padding:8px;}.items-table th{background-color:#f9f9f9;}.totals-table{width:100%;margin-top:20px;}.align-right{text-align:right;}.grand-total-row td{border-top:2px solid #333;font-weight:700}</style></head><body><div class="invoice-box"><div class="header"><h1>Quotation</h1><p>From: ${user.displayName}</p></div><div><table><tr><td><strong>Quote No:</strong> PREVIEW</td></tr><tr><td><strong>Date:</strong> ${quoteDate}</td></tr><tr><td><strong>Client:</strong> ${quoteData.clientName}</td></tr></table></div><br><table class="items-table"><thead><tr><th>#</th><th>Item</th><th>Base Price</th><th>Qty</th><th>Disc %</th><th>Disc Price</th><th>Total</th></tr></thead><tbody>${itemsHtml}</tbody></table><table class="totals-table"><tbody>${totalsHtml}</tbody></table></div></body></html>`; const options = { format: 'A4', margin: { top: '0.5in', right: '0.5in', bottom: '0.5in', left: '0.5in' } }; const pdfBuffer = await html_to_pdf.generatePdf({ content: htmlContent }, options); res.setHeader('Content-Type', 'application/pdf'); res.setHeader('Content-Disposition', `attachment; filename=Quote-Preview.pdf`); res.send(pdfBuffer); } catch (err) { console.error('Error generating preview PDF:', err); res.status(500).send('Error generating preview PDF'); } });
