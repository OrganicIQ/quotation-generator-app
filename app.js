// app.js - ADDING A DEBUG ROUTE TO CHECK LIVE ENVIRONMENT
...
// CORS Middleware
app.use(cors({
    origin: process.env.FRONTEND_URL, // This uses the URL you will set on Render
    credentials: true
}));
...
passport.use(new GoogleStrategy({ clientID: process.env.GOOGLE_CLIENT_ID, clientSecret: process.env.GOOGLE_CLIENT_SECRET, callbackURL: process.env.GOOGLE_REDIRECT_URI },
    async (accessToken, refreshToken, profile, cb) => {
        ...
        try { let user = await User.findOne({ googleId: profile.id }); if (user) { cb(null, user); } else { user = await User.create(newUser); cb(null, user); } } catch (err) { cb(err, null); }
    }
));
...
// ... (all your other routes for quotes, products, pdfs, etc. remain here)
...
// --- NEW DEBUGGING ROUTE ---
// ==========================================================
app.get('/debug-env-vars', (req, res) => {
    res.setHeader('Content-Type', 'application/json');
    res.status(200).json({
        NOTE: "This is a temporary debug route.",
        FRONTEND_URL_from_env: process.env.FRONTEND_URL || "--- VARIABLE NOT SET ---",
        GOOGLE_REDIRECT_URI_from_env: process.env.GOOGLE_REDIRECT_URI || "--- VARIABLE NOT SET ---",
    });
});
