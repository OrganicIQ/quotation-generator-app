// app.js - Final and Complete Version with All API Routes
...
// --- DATA AND ACTION API ROUTES (FULLY RESTORED) ---
...
app.post('/api/coupons/apply', ensureAuth, async (req, res) => {
    ...
    try {
        ...
        // The logic to mark coupon as used might be different depending on requirements
        // For now, we just validate and return percentage
...
app.get('/api/quotes/:id/download', ensureAuth, async (req, res) => {
    try {
        ...
        const itemsHtml = quote.lineItems.map((item, index) => { /* ... item mapping logic ... */ }).join('');
        const totalsHtml = `...`; // Totals breakdown logic
        const htmlContent = `<html>...${itemsHtml}...${totalsHtml}...</html>`;
...
// Add this entire route to your app.js file
...
app.post('/api/quotes/preview-pdf', ensureAuth, async (req, res) => {
    try {
        ...
        const itemsHtml = quoteData.lineItems.map((item, index) => {
            ...
            // The item.name already contains the full HTML from the main table
