// main.js - Updated for Multi-Category Select
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Core application logic (addProductToSelectedList, loadSavedQuotes, loadSingleQuote, loadCategories are unchanged)
    ...
    async function loadSingleQuote(quoteId) { try { const response = await fetch(`${API_BASE_URL}/api/quotes/${quoteId}`, { credentials: 'include' }); if (!response.ok) { alert('Could not retrieve quote.'); return; } const quote = await response.json(); selectedProductsTbody.innerHTML = ''; clientNameInput.value = quote.clientName; couponDiscountPercentage = quote.couponDiscountPercentage || 0; couponCodeInput.value = quote.couponCode || ''; const hasCoupon = couponDiscountPercentage > 0; lineItemDiscount = hasCoupon ? 0 : 10; couponCodeInput.disabled = hasCoupon; applyCouponBtn.disabled = hasCoupon; applyCouponBtn.textContent = hasCoupon ? 'Applied!' : 'Apply'; couponStatusMessage.textContent = hasCoupon ? `${couponDiscountPercentage}% coupon applied.` : ''; if(hasCoupon) { couponStatusMessage.style.color = 'var(--primary-green)'; } quote.lineItems.forEach((item, index) => { const product = item.product; if (!product) return; const rowIndex = index + 1; const { priceAfterDiscount, total } = calculateQuotation(item.priceAtTime, item.quantity, item.discountPercentage); const itemNameHtml = `<strong>${product.baseName} ${product.variantName ? `(${product.variantName})` : ''}</strong><br><small>${product.description || ''}</small>`; const newRow = document.createElement('tr'); newRow.classList.add('selected-product-row'); newRow.dataset.productId = product._id; newRow.innerHTML = `<td>${rowIndex}</td><td>${itemNameHtml}</td><td>${item.priceAtTime.toFixed(2)}</td><td><input type="number" class="quantity-input-small" value="${item.quantity}" min="1" data-base-price="${item.priceAtTime}"/></td><td>${item.discountPercentage}%</td><td><span class="item-discounted-price">${priceAfterDiscount.toFixed(2)}</span></td><td><span class="item-total-price">${total.toFixed(2)}</span></td><td><button class="remove-btn">Remove</button></td>`; selectedProductsTbody.appendChild(newRow); newRow.querySelector('.quantity-input-small').addEventListener('input', (event) => { let quantity = parseInt(event.target.value) || 1; const basePrice = parseFloat(event.target.dataset.basePrice); const { priceAfterDiscount, total } = calculateQuotation(basePrice, quantity, item.discountPercentage); newRow.querySelector('.item-discounted-price').textContent = priceAfterDiscount.toFixed(2); newRow.querySelector('.item-total-price').textContent = total.toFixed(2); updateFinalTotals(); }); newRow.querySelector('.remove-btn').addEventListener('click', () => { newRow.remove(); updateFinalTotals(); selectedProductsTbody.querySelectorAll('tr').forEach((r, idx) => { r.cells[0].textContent = idx + 1; }); }); }); noItemsMessage.style.display = 'none'; selectedProductsTable.style.display = 'table'; updateFinalTotals(); window.scrollTo({ top: 0, behavior: 'smooth' }); } catch (error) { console.error("Error loading single quote:", error); } }
    async function loadCategories() { try { const response = await fetch(`${API_BASE_URL}/api/categories`, { credentials: 'include' }); if (!response.ok) return; const categories = await response.json(); categoryPillsContainer.innerHTML = ''; const allPill = document.createElement('div'); allPill.classList.add('category-pill', 'active'); allPill.textContent = 'All Categories'; allPill.dataset.category = ''; categoryPillsContainer.appendChild(allPill); categories.forEach(category => { if (category) { const pill = document.createElement('div'); pill.classList.add('category-pill'); pill.textContent = category; pill.dataset.category = category; categoryPillsContainer.appendChild(pill); } }); } catch (error) { console.error('Error loading categories:', error); } }

    // --- MODIFIED SECTION: SEARCH AND FILTER LOGIC ---
    ...
    const performSearch = async (searchTerm, categories) => {
        ...
        // MODIFIED: Append each selected category to the query
    ...
    const handleSearchAndFilter = () => {
        ...
        // MODIFIED: Collect all active categories into an array
        const activePillNodes = categoryPillsContainer.querySelectorAll('.category-pill.active');
        const categories = Array.from(activePillNodes).map(pill => pill.dataset.category).filter(cat => cat !== ''); // a.dataset.category' gets the category name. We also filter out the empty one for "All categories"

        // If the categories array is empty (which means 'All Categories' is selected), we pass an empty array to the search
        debouncedSearch(searchTerm, categories);
    };

    quoteSearchInput.addEventListener('input', handleSearchAndFilter);

    // MODIFIED: New click handling logic for category pills
    categoryPillsContainer.addEventListener('click', (event) => {
        const clickedPill = event.target;
        if (!clickedPill.classList.contains('category-pill')) return;

        const allCategoriesPill = categoryPillsContainer.querySelector('[data-category=""]');
        const isAllCategories = clickedPill === allCategoriesPill;

        if (isAllCategories) {
            // If "All Categories" is clicked, activate it and deactivate all others.
            ...
            // If any other pill is clicked, deactivate "All Categories" and toggle the clicked one.
            allCategoriesPill.classList.remove('active');
            clickedPill.classList.toggle('active');
        }

        // If no specific categories are selected, re-activate the "All Categories" pill.
        const anyActive = categoryPillsContainer.querySelector('.category-pill.active:not([data-category=""])');
        if (!anyActive) {
            allCategoriesPill.classList.add('active');
        }
    ...
    // Login/Logout and Save/Download logic (no changes here)
    ...
    if (saveQuoteBtn) { saveQuoteBtn.addEventListener('click', async () => { const productRows = selectedProductsTbody.querySelectorAll('tr.selected-product-row'); if (productRows.length === 0) { alert('Cannot save empty quote.'); return; } const lineItems = Array.from(productRows).map(row => ({ product: row.dataset.productId, quantity: parseInt(row.querySelector('.quantity-input-small').value), priceAtTime: parseFloat(row.cells[2].textContent), discountPercentage: parseFloat(row.cells[4].textContent) || 0 })); const subtotal = parseFloat(subtotalAmountSpan.textContent); const discountAmount = parseFloat(discountAmountSpan.textContent) || 0; const gstAmount = parseFloat(gstAmountSpan.textContent); const grandTotal = parseFloat(grandTotalAmountSpan.textContent); const quoteData = { clientName: clientNameInput.value.trim() || 'N/A', lineItems, subtotal, couponCode: couponDiscountPercentage > 0 ? couponCodeInput.value.trim() : null, couponDiscountPercentage, couponDiscountAmount: discountAmount, gstPercentage: GST_RATE, gstAmount, grandTotal }; try { const response = await fetch(`${API_BASE_URL}/api/quotes`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(quoteData), credentials: 'include' }); if (response.ok) { const result = await response.json(); alert(`Quote ${result.quoteNumber} saved successfully!`); loadSavedQuotes(); } else { const errorData = await response.json(); alert(`Error: ${errorData.message}`); } } catch (err) { console.error("Error saving quote:", err); alert('Error saving quote.'); } }); }
    if (downloadPdfBtn) { downloadPdfBtn.addEventListener('click', async () => { const productRows = selectedProductsTbody.querySelectorAll('tr.selected-product-row'); if (productRows.length === 0) { alert('No items to download.'); return; } const lineItems = Array.from(productRows).map(row => ({ name: row.cells[1].innerHTML, quantity: parseInt(row.querySelector('.quantity-input-small').value), price: parseFloat(row.cells[2].textContent), discountPercentage: parseFloat(row.cells[4].textContent) || 0 })); const quoteData = { clientName: clientNameInput.value.trim() || 'N/A', lineItems, subtotal: parseFloat(subtotalAmountSpan.textContent), couponDiscountPercentage, couponDiscountAmount: parseFloat(discountAmountSpan.textContent) || 0, gstPercentage: GST_RATE, gstAmount: parseFloat(gstAmountSpan.textContent), grandTotal: parseFloat(grandTotalAmountSpan.textContent) }; try { const response = await fetch(`${API_BASE_URL}/api/quotes/preview-pdf`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(quoteData), credentials: 'include' }); if (!response.ok) { throw new Error('Server failed to generate PDF.'); } const blob = await response.blob(); const url = window.URL.createObjectURL(blob); const a = document.createElement('a'); a.style.display = 'none'; a.href = url; a.download = `Quote-Preview_${Date.now()}.pdf`; document.body.appendChild(a); a.click(); window.URL.revokeObjectURL(url); a.remove(); } catch (err) { console.error("PDF Preview Error:", err); alert("Could not generate PDF preview."); } }); }
    if (downloadExcelBtn) { downloadExcelBtn.addEventListener('click', () => { const productRows = selectedProductsTbody.querySelectorAll('tr.selected-product-row'); if (productRows.length === 0) { alert('No items to download.'); return; } const data = [['#', 'Item', 'Base Price', 'Qty', 'Disc %', 'Disc Price', 'Total']]; productRows.forEach(row => { const cells = row.querySelectorAll('td'); data.push([ cells[0].textContent, cells[1].innerText.replace(/\n/g, " "), cells[2].textContent, cells[3].querySelector('input').value, cells[4].textContent, cells[5].textContent, cells[6].textContent ]); }); data.push([]); data.push(['', '', '', '', '', 'Subtotal', subtotalAmountSpan.textContent]); if (couponDiscountPercentage > 0) { data.push(['', '', '', '', '', `Discount (${couponDiscountPercentage}%)`, `-${discountAmountSpan.textContent}`]); } data.push(['', '', '', '', '', `GST (${GST_RATE}%)`, `+${gstAmountSpan.textContent}`]); data.push(['', '', '', '', '', 'Grand Total', grandTotalAmountSpan.textContent]); const workbook = XLSX.utils.book_new(); const worksheet = XLSX.utils.aoa_to_sheet(data); XLSX.utils.book_append_sheet(workbook, worksheet, 'Quotation'); XLSX.writeFile(workbook, `Quotation_${new Date().toISOString().slice(0, 10)}.xlsx`); }); }
    if (applyCouponBtn) { applyCouponBtn.addEventListener('click', async () => { const code = couponCodeInput.value.trim(); if (!code) { couponStatusMessage.textContent = 'Please enter a code.'; couponStatusMessage.style.color = 'var(--danger-color)'; return; } try { const response = await fetch(`${API_BASE_URL}/api/coupons/apply`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ code: code }), credentials: 'include' }); const data = await response.json(); if (response.ok) { couponStatusMessage.textContent = data.message; couponStatusMessage.style.color = 'var(--primary-green)'; couponDiscountPercentage = data.discountPercentage; lineItemDiscount = 0; recalculateAllRows(); couponCodeInput.disabled = true; applyCouponBtn.disabled = true; applyCouponBtn.textContent = 'Applied!'; } else { couponStatusMessage.textContent = data.message; couponStatusMessage.style.color = 'var(--danger-color)'; } } catch (err) { couponStatusMessage.textContent = 'An error occurred.'; couponStatusMessage.style.color = 'var(--danger-color)'; } }); }
});