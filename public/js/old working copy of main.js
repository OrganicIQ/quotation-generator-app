// main.js - FINAL VERSION WITH ALL EVENT LISTENERS RESTORED

// This line automatically detects the environment and sets the correct backend URL.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    savedQuotesList.addEventListener('click',async(e)=>{const t=e.target;if(t.matches('.delete-quote-btn')){const qId=t.dataset.quoteId;if(confirm('Are you sure?')){try{const r=await fetch(`${API_BASE_URL}/api/quotes/${qId}`,{method:'DELETE',credentials:'include'});if(r.ok){t.closest('.saved-quote-item').remove()}else{alert('Failed to delete.')}}catch(e){alert('Error deleting.')}}}else if(t.matches('.download-pdf-btn')){const qId=t.dataset.quoteId;window.open(`${API_BASE_URL}/api/quotes/${qId}/download`,'_blank')}else if(t.closest('.quote-item-details-wrapper')){const qId=t.closest('.quote-item-details-wrapper').dataset.quoteId;loadSingleQuote(qId)}});
    const performSearch=async(sT)=>{autocompleteResultsUl.innerHTML='';if(!sT)return;try{const r=await fetch(`${API_BASE_URL}/api/products?q=${encodeURIComponent(sT)}`);const p=await r.json();if(p.length>0){p.forEach(prod=>{const li=document.createElement('li');li.textContent=`${prod.baseName} - ₹${prod.basePrice.toFixed(2)}`;li.dataset.product=JSON.stringify(prod);li.addEventListener('click',()=>{addProductToSelectedList(JSON.parse(li.dataset.product));quoteSearchInput.value='';autocompleteResultsUl.innerHTML=''});autocompleteResultsUl.appendChild(li)})}else{autocompleteResultsUl.innerHTML='<li>No items found</li>'}autocompleteResultsUl.classList.add('visible')}catch(e){console.error('Error searching:',e)}};
    quoteSearchInput.addEventListener('input',(e)=>debounce(performSearch,300)(e.target.value.trim()));
    if(saveQuoteBtn){saveQuoteBtn.addEventListener('click',async()=>{const pR=selectedProductsTbody.querySelectorAll('tr.selected-product-row');if(pR.length===0){alert('Cannot save empty quote.');return;}const lI=Array.from(pR).map(row=>({product:row.dataset.productId,quantity:parseInt(row.querySelector('.quantity-input-small').value),priceAtTime:parseFloat(row.cells[2].textContent),discountPercentage:parseFloat(row.cells[4].textContent)||0}));const subtotal=parseFloat(subtotalAmountSpan.textContent);const discountAmount=parseFloat(discountAmountSpan.textContent)||0;const gstAmount=parseFloat(gstAmountSpan.textContent);const grandTotal=parseFloat(grandTotalAmountSpan.textContent);const qD={clientName:clientNameInput.value.trim()||'N/A',lineItems:lI,subtotal,couponCode:couponDiscountPercentage>0?couponCodeInput.value.trim():null,couponDiscountPercentage,couponDiscountAmount:discountAmount,gstAmount,grandTotal};try{const r=await fetch(`${API_BASE_URL}/api/quotes`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(qD),credentials:'include'});if(r.ok){const res=await r.json();alert(`Quote ${res.quoteNumber} saved!`);loadSavedQuotes()}else{const err=await r.json();alert(`Error: ${err.message}`)}}catch(e){alert('Error saving.')}})}
    if(downloadPdfBtn){downloadPdfBtn.addEventListener('click',async()=>{const pR=selectedProductsTbody.querySelectorAll('tr.selected-product-row');if(pR.length===0){alert('No items to download.');return;}const lI=Array.from(pR).map(row=>({name:row.cells[1].innerHTML,description:'',quantity:parseInt(row.querySelector('.quantity-input-small').value),price:parseFloat(row.cells[2].textContent),discountPercentage:parseFloat(row.cells[4].textContent)||0}));const qD={clientName:clientNameInput.value.trim()||'N/A',lineItems:lI,subtotal:parseFloat(subtotalAmountSpan.textContent),couponDiscountPercentage:couponDiscountPercentage,couponDiscountAmount:parseFloat(discountAmountSpan.textContent)||0,gstPercentage:GST_RATE,gstAmount:parseFloat(gstAmountSpan.textContent),grandTotal:parseFloat(grandTotalAmountSpan.textContent)};try{const r=await fetch(`${API_BASE_URL}/api/quotes/preview-pdf`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(qD),credentials:'include'});if(!r.ok){throw new Error('Server failed to generate PDF.')}const b=await r.blob();const u=window.URL.createObjectURL(b);const a=document.createElement('a');a.style.display='none';a.href=u;a.download=`Quote-Preview_${Date.now()}.pdf`;document.body.appendChild(a);a.click();window.URL.revokeObjectURL(u);a.remove()}catch(e){console.error("PDF Preview Error:",e);alert("Could not generate PDF preview.")}})}
    if(downloadExcelBtn){downloadExcelBtn.addEventListener('click',()=>{const pR=selectedProductsTbody.querySelectorAll('tr.selected-product-row');if(pR.length===0){alert('No items to download.');return}const d=[['#','Item','Base Price','Qty','Disc %','Disc Price','Total']];pR.forEach(row=>{const c=row.querySelectorAll('td');d.push([c[0].textContent,c[1].textContent.replace(/<br\s*[\/]?>/gi," ").replace(/<[^>]+>/g,''),c[2].textContent,c[3].querySelector('input').value,c[4].textContent,c[5].textContent,c[6].textContent])});d.push([]);d.push(['','','','','','Subtotal',subtotalAmountSpan.textContent]);d.push(['','','','','',`Discount (${couponDiscountPercentage}%)`,discountAmountSpan.textContent]);d.push(['','','','','','GST (18%)',gstAmountSpan.textContent]);d.push(['','','','','','Grand Total',grandTotalAmountSpan.textContent]);const wB=XLSX.utils.book_new();const wS=XLSX.utils.aoa_to_sheet(d);XLSX.utils.book_append_sheet(wB,wS,'Quotation');XLSX.writeFile(wB,`Quotation_${new Date().toISOString().slice(0,10)}.xlsx`)})}
    
    // --- THIS IS THE CORRECTED COUPON CODE BLOCK ---
    if (applyCouponBtn) {
        applyCouponBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    lineItemDiscount = 0; // Remove standard discount
                    recalculateAllRows(); // Recalculate everything
                ...
                couponStatusMessage.textContent = 'An error occurred. Please try again.';
                couponStatusMessage.style.color = 'var(--danger-color)';
            }
        });
    }
